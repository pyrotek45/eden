...
fn test_command_manager_push_undo() {
    ...
    mgr.undo(&mut project);
    assert!((project.tracks[0].volume - 1.0).abs() < 1e-6);
}
...
fn test_project_clone_is_deep() {
    ...
    assert!(
        (original.tracks[0].volume - 1.0).abs() < 1e-6,
...
fn make_audio_clip_project(sample_rate: u32, duration_secs: f64, freq: f64, bpm: f64) -> Project {
...
fn render_audio_clip_project(
    ...
    use crate::engine::AudioSampleClip;
    use std::sync::Arc;
...
fn test_limiter_render_no_clicks() {
    ...
    assert!(
        max_j < 0.45,
        "Limiter render has clicks: max jump {}",
        max_j
    );
}
...
fn test_delay_render_no_clicks() {
    ...
    let skip = 128;
    let max_j = max_sample_jump(&buf[skip..]);
    assert!(max_j < 0.45, "Delay render has clicks: max jump {}", max_j);
}
...
fn test_reverb_render_no_clicks() {
    ...
    let skip = 128;
    let max_j = max_sample_jump(&buf[skip..]);
    assert!(max_j < 0.45, "Reverb render has clicks: max jump {}", max_j);
}
...
fn test_chorus_render_no_clicks() {
    ...
    let skip = 128;
    let max_j = max_sample_jump(&buf[skip..]);
    assert!(max_j < 0.45, "Chorus render has clicks: max jump {}", max_j);
}
...
fn test_limiter_no_clicks() {
    ...
    let skip = 256;
    let max_j = max_sample_jump(&buf[skip..]);
    assert!(max_j < 0.45, "Limiter has clicks: max jump {}", max_j);
}
...
fn test_compressor_no_clicks() {
    ...
    let skip = 256;
    let max_j = max_sample_jump(&buf[skip..]);
    assert!(max_j < 0.45, "Compressor has clicks: max jump {}", max_j);
}
...
fn has_denormals(buf: &[(f64, f64)]) -> bool {
    buf.iter().any(|(l, r)| {
        ...
        (lf != 0.0 && lf.abs() < f32::MIN_POSITIVE) || (rf != 0.0 && rf.abs() < f32::MIN_POSITIVE)
    })
}
...
#[test]
fn test_limiter_output_at_ceiling() {
    // Feed constant +6 dB (amplitude 2.0) into the limiter with:
    //   gain_db = 0, ceiling_db = 0, output_db = 0, release = 0.05
    // The limiter should clamp the output to ceiling = 1.0 (0 dB).
    ...
    let input = 2.0_f64; // +6 dB

    // Prime: run 500 samples to fill lookahead buffer and stabilize
    for _ in 0..500 {
        lim.process(input, input, &params, sr);
    }

    // Measure: run 1000 samples and check output is at ceiling
    let mut max_out = 0.0_f64;
    let mut min_out = f64::MAX;
    for _ in 0..1000 {
        let (ol, or_) = lim.process(input, input, &params, sr);
        max_out = max_out.max(ol.abs());
        min_out = min_out.min(ol.abs());
        // Also check right channel
        assert!(
            (or_.abs() - 1.0).abs() < 0.05,
            "Right channel should be at ceiling (1.0), got {}",
            or_
        );
    }

    eprintln!(
        "Limiter test: max_out={:.6}, min_out={:.6}",
        max_out, min_out
    );
    assert!(
        (max_out - 1.0).abs() < 0.05,
        "Max output should be ~1.0 (ceiling), got {:.6}",
        max_out
    );
    assert!(
        (min_out - 1.0).abs() < 0.05,
        "Min output should be ~1.0 (ceiling), got {:.6}",
        min_out
    );
}
...
/// Helper: build a "fully loaded" project with every field set to non-default.
fn make_maxed_project() -> Project {
    let mut p = Project {
        name: "MaxedProject".into(),
        sample_rate: 96000,
        tracks: Vec::new(),
        tempo_map: TempoMap {
            changes: vec![
    ...
    // MIDI track with all fields populated
    let mut t1 = Track::new(1, "FullMidi", TrackType::Midi);
    t1.volume = 0.73;
    t1.pan = -0.4;
    t1.mute = true;
    t1.solo = true;
    t1.color = [10, 20, 30, 40];
    t1.height = 120;
    t1.instrument_idx = 3;
    t1.sampler_file = Some("/path/to/sample.wav".into());
    t1.automation_enabled = false;
    t1.cstrip2_params = vec![("treble".into(), 0.7), ("bass".into(), 0.3)];
    t1.cstrip2_bypass = true;
    t1.rack = vec![
        RackSlot::subtractive_synth(100),
        RackSlot::delay(101),
        RackSlot::reverb(102),
    ];
    // Sidechain on the delay
    t1.rack[1].sidechain_track_id = Some(2);
    t1.rack[1].enabled = false;
    // Tweak a param
    if let Some(p) = t1.rack[0].params.iter_mut().find(|p| p.id == "gain") {
        p.value = -12.0;
    }
    ...
    // Audio track
    let mut t2 = Track::new(2, "FullAudio", TrackType::Audio);
    t2.volume = 0.0;
    t2.pan = 1.0;
    t2.clips.push(Clip::Audio(AudioClip {
        source_file: "/path/to/audio.wav".into(),
        start_time: 2.5,
        offset: 1.3,
        length: 16.0,
        gain: 0.5,
        name: "AudioClip1".into(),
        color: [0, 255, 0, 200],
        fade_in: 0.1,
        fade_out: 0.5,
    }));
    ...
    // Automation track
    let mut t3 = Track::new(3, "FullAuto", TrackType::Automation);
    t3.clips.push(Clip::Automation(AutomationClip {
        points: vec![
            AutomationPoint {
                time: 0.0,
...
#[test]
fn test_save_load_truncated_json_error() {
    let result: Result<Project, _> = serde_json::from_str(r#"{"name": "Test", "tracks":"#);
    assert!(result.is_err());
}

// ═══════════════════════════════════════════════════════════════
// LIMITER HARD CLAMP TESTS
// ═══════════════════════════════════════════════════════════════

#[test]
fn test_limiter_hard_clamp_with_output_gain() {
    // The limiter's output_db can push signal above ceiling.
    // After the hard clamp fix, output must NEVER exceed ceiling.
    let mut lim = FxLimiter::new();
    let sr = 44100.0;
    let params: Vec<(String, f32)> = vec![
        ("gain_db".to_string(), 12.0),    // hot input
        ("ceiling_db".to_string(), -3.0), // ceiling at -3 dB
        ("release".to_string(), 0.05),
        ("output_db".to_string(), 12.0), // +12 dB output — would push above ceiling
    ];
    let ceiling_lin = 10.0_f64.powf(-3.0 / 20.0);
    let input = 1.0_f64; // 0 dBFS input + 12 dB gain = very hot
                         // Prime
    for _ in 0..1000 {
        lim.process(input, input, &params, sr);
    }
    ...
    let mut max_out = 0.0_f64;
    for _ in 0..2000 {
        let (l, r) = lim.process(input, input, &params, sr);
        max_out = max_out.max(l.abs()).max(r.abs());
    }
    assert!(
        max_out <= ceiling_lin + 1e-6,
        "Limiter output ({:.6}) must not exceed ceiling ({:.6})",
        max_out,
        ceiling_lin,
    );
}
...
#[test]
fn test_limiter_hard_clamp_transients() {
    // Send sharp transients — output must never exceed ceiling
    let mut lim = FxLimiter::new();
    let sr = 44100.0;
    let params: Vec<(String, f32)> = vec![
        ("gain_db".to_string(), 0.0),
        ("ceiling_db".to_string(), -6.0),
        ("release".to_string(), 0.001),
        ("output_db".to_string(), 0.0),
    ];
    let ceiling_lin = 10.0_f64.powf(-6.0 / 20.0);
    let mut max_out = 0.0_f64;
    // Alternate between silence and max-amplitude bursts
    for i in 0..10000 {
        let input = if (i / 50) % 2 == 0 { 4.0 } else { 0.0 };
        let (l, r) = lim.process(input, -input, &params, sr);
        max_out = max_out.max(l.abs()).max(r.abs());
    }
    assert!(
        max_out <= ceiling_lin + 1e-6,
        "Limiter transient output ({:.6}) exceeds ceiling ({:.6})",
        max_out,
        ceiling_lin,
    );
}
...
#[test]
fn test_limiter_hard_clamp_many_hot_tracks_simulated() {
    // Simulate 5 maxed-out tracks summed → limiter should hold ceiling
    let mut lim = FxLimiter::new();
    let sr = 44100.0;
    let params: Vec<(String, f32)> = vec![
        ("gain_db".to_string(), 0.0),
        ("ceiling_db".to_string(), 0.0),
        ("release".to_string(), 0.05),
        ("output_db".to_string(), 0.0),
    ];
    let ceiling_lin = 1.0_f64;
    // 5 tracks each at +0 dBFS = 5.0 linear
    let hot_signal = 5.0_f64;
    // Prime
    for _ in 0..500 {
        lim.process(hot_signal, hot_signal, &params, sr);
    }
    let mut max_out = 0.0_f64;
    for _ in 0..2000 {
        let (l, r) = lim.process(hot_signal, hot_signal, &params, sr);
        max_out = max_out.max(l.abs()).max(r.abs());
    }
    assert!(
        max_out <= ceiling_lin + 1e-6,
        "5 hot tracks through limiter: max={:.6}, ceiling={:.6}",
        max_out,
        ceiling_lin,
    );
}
...
fn test_limiter_negative_ceiling() {
    // Ceiling at -12 dB — output must never exceed that
    let mut lim = FxLimiter::new();
    let sr = 44100.0;
    let params: Vec<(String, f32)> = vec![
        ("gain_db".to_string(), 24.0),
        ("ceiling_db".to_string(), -12.0),
        ("release".to_string(), 0.05),
        ("output_db".to_string(), 0.0),
    ];
    let ceiling_lin = 10.0_f64.powf(-12.0 / 20.0);
    for _ in 0..500 {
        lim.process(1.0, 1.0, &params, sr);
    }
    let mut max_out = 0.0_f64;
    for _ in 0..2000 {
        let (l, r) = lim.process(1.0, 1.0, &params, sr);
        max_out = max_out.max(l.abs()).max(r.abs());
    }
    assert!(
        max_out <= ceiling_lin + 1e-6,
        "Limiter at -12dB ceiling: max={:.6}, ceiling={:.6}",
        max_out,
        ceiling_lin,
    );
}
...
fn test_limiter_silence_passthrough() {
    // Zero input → zero output (no noise floor)
    let mut lim = FxLimiter::new();
    let sr = 44100.0;
    let params: Vec<(String, f32)> = vec![
        ("gain_db".to_string(), 0.0),
    ...
    for _ in 0..500 {
        lim.process(0.0, 0.0, &params, sr);
    }
    let mut max_out = 0.0_f64;
    for _ in 0..500 {
        let (l, r) = lim.process(0.0, 0.0, &params, sr);
        max_out = max_out.max(l.abs()).max(r.abs());
    }
    assert!(
        max_out < 1e-10,
        "Silence through limiter should produce silence, got {:.12}",
        max_out,
    );
}

#[test]
fn test_limiter_below_ceiling_passes_unchanged() {
    // Signal below ceiling should pass through unmodified
    let mut lim = FxLimiter::new();
    let sr = 44100.0;
    let params: Vec<(String, f32)> = vec![
        ("gain_db".to_string(), 0.0),
    ...
    let input = 0.5_f64; // -6 dBFS, well below 0 dB ceiling
                         // Prime lookahead
    for _ in 0..500 {
        lim.process(input, input, &params, sr);
    }
    // Measure steady state — should be very close to input
    let mut total_diff = 0.0_f64;
    let n = 1000;
    for _ in 0..n {
        let (l, _r) = lim.process(input, input, &params, sr);
        total_diff += (l - input).abs();
    }
    let avg_diff = total_diff / n as f64;
    assert!(
        avg_diff < 0.01,
        "Signal below ceiling should pass through ~unchanged, avg diff={:.6}",
        avg_diff,
    );
}
...
#[test]
fn test_render_volume_scales_linearly() {
    // Render at vol=1.0 vs vol=0.5: the half-volume render should
    // have roughly half the RMS energy.
    let mut p = make_test_project();
    // Remove master rack so limiter doesn't compress the difference
    p.master_rack.clear();
    p.tracks[0].volume = 1.0;
    let buf_full = render_to_buffer(&p, 44100, 1.0);
    p.tracks[0].volume = 0.5;
    let buf_half = render_to_buffer(&p, 44100, 1.0);

    let rms = |buf: &[(f64, f64)]| -> f64 {
        let sum: f64 = buf.iter().map(|(l, r)| l * l + r * r).sum();
        (sum / buf.len().max(1) as f64).sqrt()
    };
    let rms_full = rms(&buf_full);
    let rms_half = rms(&buf_half);
    if rms_full > 0.001 {
        let ratio = rms_half / rms_full;
        assert!(
            (ratio - 0.5).abs() < 0.15,
            "Volume 0.5 should give ~0.5× RMS, got ratio={:.3}",
            ratio,
        );
    }
}
...
#[test]
fn test_render_pan_hard_left() {
    // Pan = -1.0 → all signal in left channel, near-zero in right
    let mut p = make_test_project();
    p.master_rack.clear();
    p.tracks[0].pan = -1.0;
    let buf = render_to_buffer(&p, 44100, 1.0);
    let sum_r: f64 = buf.iter().map(|(_, r)| r.abs()).sum();
    let sum_l: f64 = buf.iter().map(|(l, _)| l.abs()).sum();
    if sum_l > 0.01 {
        assert!(
            sum_r / sum_l < 0.05,
            "Hard left pan: right channel should be near-zero, ratio={:.4}",
            sum_r / sum_l,
        );
    }
}
...
fn test_render_pan_hard_right() {
    ...
    if sum_r > 0.01 {
        assert!(
            sum_l / sum_r < 0.05,
            "Hard right pan: left channel should be near-zero, ratio={:.4}",
            sum_l / sum_r,
        );
    }
}
...
#[test]
fn test_render_mute_vs_zero_volume() {
    // Muted track and zero-volume track should both produce silence
    let mut p_mute = make_test_project();
    p_mute.master_rack.clear();
    p_mute.tracks[0].mute = true;
    let buf_mute = render_to_buffer(&p_mute, 44100, 1.0);

    let mut p_zero = make_test_project();
    p_zero.master_rack.clear();
    p_zero.tracks[0].volume = 0.0;
    let buf_zero = render_to_buffer(&p_zero, 44100, 1.0);

    let max_mute: f64 = buf_mute
        .iter()
        .map(|(l, r)| l.abs().max(r.abs()))
        .fold(0.0, f64::max);
    let max_zero: f64 = buf_zero
        .iter()
        .map(|(l, r)| l.abs().max(r.abs()))
        .fold(0.0, f64::max);
    assert!(
        max_mute < 1e-6,
        "Muted track should be silent, max={:.8}",
        max_mute
    );
    assert!(
        max_zero < 1e-6,
        "Zero-volume should be silent, max={:.8}",
        max_zero
    );
}

#[test]
fn test_render_solo_isolates_track() {
    // With solo on track 1, track 2 should not contribute
    let mut p = Project::default();
    p.master_rack.clear();
    let mut t1 = Track::new(1, "A", TrackType::Midi);
    t1.rack = vec![RackSlot::sine_osc(1)];
    t1.solo = true;
    t1.clips.push(Clip::Midi(MidiClip {
        notes: vec![MidiNote {
            pitch: 69,
    ...
    p.tracks.push(t1);
    let mut t2 = Track::new(2, "B", TrackType::Midi);
    t2.rack = vec![RackSlot::sine_osc(1)];
    ...
    let buf_solo = render_to_buffer(&p, 44100, 1.0);
    // Now render with only track 1 (remove track 2)
    p.tracks.remove(1);
    p.tracks[0].solo = false; // no solo needed with only 1 track
    let buf_single = render_to_buffer(&p, 44100, 1.0);

    // Both should be the same length (determined by clips)
    let len = buf_solo.len().min(buf_single.len());
    if len > 0 {
        let mut diff_sum = 0.0_f64;
        for i in 0..len {
            diff_sum += (buf_solo[i].0 - buf_single[i].0).abs();
            diff_sum += (buf_solo[i].1 - buf_single[i].1).abs();
        }
        let avg_diff = diff_sum / (len * 2) as f64;
        assert!(
            avg_diff < 0.01,
            "Solo should isolate track — avg diff between solo and single: {:.6}",
            avg_diff,
        );
    }
}
...
#[test]
fn test_render_master_volume_halves_output() {
    let p = make_test_project();
    let buf_full = render_to_buffer(&p, 44100, 1.0);
    let buf_half = render_to_buffer(&p, 44100, 0.5);
    let rms = |buf: &[(f64, f64)]| -> f64 {
        let sum: f64 = buf.iter().map(|(l, r)| l * l + r * r).sum();
        (sum / buf.len().max(1) as f64).sqrt()
    };
    ...
    if r1 > 0.001 {
        let ratio = r2 / r1;
        // Should be roughly 0.5 (limiter may compress the full version)
        assert!(
            ratio < 0.8,
            "Master volume 0.5 should reduce level, ratio={:.3}",
            ratio,
        );
    }
}
...
#[test]
fn test_render_limiter_prevents_clipping() {
    // Multiple loud tracks through the default limiter — output must stay ≤ 1.0
    let mut p = Project::default();
    // Default master rack has a limiter
    for i in 0..5 {
        let mut t = Track::new(i + 1, &format!("Hot{}", i), TrackType::Midi);
        t.volume = 1.0;
...
#[test]
fn test_render_no_nan_or_inf() {
    let p = make_maxed_project();
    let buf = render_to_buffer(&p, 44100, 1.0);
    for (i, (l, r)) in buf.iter().enumerate() {
        assert!(l.is_finite(), "NaN/Inf in left channel at sample {}", i);
        assert!(r.is_finite(), "NaN/Inf in right channel at sample {}", i);
    }
}

#[test]
fn test_render_disabled_effect_is_bypassed() {
    let mut p = make_test_project();
    p.master_rack.clear();
    // Add a heavy distortion with extreme settings — disabled
    let mut dist = RackSlot::distortion(10);
    dist.enabled = false;
    if let Some(drv) = dist.params.iter_mut().find(|p| p.id == "drive") {
        drv.value = 1.0;
    }
    p.tracks[0].rack.push(dist);
    let buf_with = render_to_buffer(&p, 44100, 1.0);

    // Now without the slot at all
    p.tracks[0].rack.pop();
    ...
    for i in 0..len {
        diff += (buf_with[i].0 - buf_without[i].0).abs();
    }
    let avg = diff / len.max(1) as f64;
    assert!(
        avg < 0.001,
        "Disabled effect should be bypassed, avg diff={:.6}",
        avg,
    );
}

#[test]
fn test_render_effects_chain_order_matters() {
    // Distortion then reverb should differ from reverb then distortion
    let mut p1 = make_test_project();
    p1.master_rack.clear();
    p1.tracks[0].rack.push(RackSlot::distortion(10));
    p1.tracks[0].rack.push(RackSlot::reverb(11));
    let buf1 = render_to_buffer(&p1, 44100, 1.0);

    let mut p2 = make_test_project();
    p2.master_rack.clear();
    p2.tracks[0].rack.push(RackSlot::reverb(10));
    p2.tracks[0].rack.push(RackSlot::distortion(11));
    let buf2 = render_to_buffer(&p2, 44100, 1.0);

    let len = buf1.len().min(buf2.len());
    let mut diff = 0.0_f64;
    for i in 0..len {
        diff += (buf1[i].0 - buf2[i].0).abs();
    }
    let avg = diff / len.max(1) as f64;
    // They should be different — effect order matters
    assert!(
        avg > 0.0001,
        "Effect chain order should matter, avg diff={:.8}",
        avg,
    );
}

#[test]
fn test_render_deterministic2() {
    // Same project rendered twice should produce identical output
    let p = make_test_project();
    ...
    assert_eq!(
        buf1.len(),
        buf2.len(),
        "Render length should be deterministic"
    );
    for i in 0..buf1.len() {
        assert!(
            (buf1[i].0 - buf2[i].0).abs() < 1e-10 && (buf1[i].1 - buf2[i].1).abs() < 1e-10,
            "Render not deterministic at sample {}: ({:.8},{:.8}) vs ({:.8},{:.8})",
            i,
            buf1[i].0,
            buf1[i].1,
            buf2[i].0,
...
#[test]
fn test_render_automation_track_no_audio() {
    // A project with only an automation track should be silent
    let mut p = Project::default();
    p.master_rack.clear();
    let mut t = Track::new(1, "Auto", TrackType::Automation);
    ...
    let max_abs = buf
        ...
        .map(|(l, r)| l.abs().max(r.abs()))
        .fold(0.0_f64, f64::max);
    assert!(max_abs < 1e-6, "Automation-only project should be silent");
}

#[test]
fn test_render_all_tracks_muted_is_silent2() {
    let mut p = make_test_project();
    ...
    assert!(
        max_abs < 1e-6,
        "All muted should be silent, max={:.8}",
        max_abs
    );
}

#[test]
fn test_render_master_volume_zero_is_silent2() {
    let p = make_test_project();
    let buf = render_to_buffer(&p, 44100, 0.0);
    let max_abs = buf
        .iter()
        .map(|(l, r)| l.abs().max(r.abs()))
        .fold(0.0_f64, f64::max);
    assert!(
        max_abs < 1e-6,
        "Master volume 0 should be silent, max={:.8}",
        max_abs
    );
}

// ═══════════════════════════════════════════════════════════════
// SIGNAL CHAIN PARITY TESTS (render vs expected behavior)
// ═══════════════════════════════════════════════════════════════

#[test]
fn test_render_center_pan_equal_power() {
    // Center pan: L and R should have equal power
    let mut p = make_test_project();
    p.master_rack.clear();
    p.tracks[0].pan = 0.0;
    let buf = render_to_buffer(&p, 44100, 1.0);
    let sum_l: f64 = buf.iter().map(|(l, _)| l * l).sum();
    let sum_r: f64 = buf.iter().map(|(_, r)| r * r).sum();
    if sum_l > 0.001 {
        let ratio = sum_r / sum_l;
        assert!(
            (ratio - 1.0).abs() < 0.1,
            "Center pan should give equal L/R power, ratio={:.4}",
            ratio,
        );
    }
}
...
#[test]
fn test_render_sqrt2_compensation() {
    // At center pan with a mono source, left and right channels
    // should each have approximately the same amplitude as the source
    // (due to √2 compensation: cos(π/4)·√2 = 1.0)
    let mut p = Project::default();
    p.master_rack.clear();
    let mut t = Track::new(1, "S", TrackType::Midi);
    t.rack = vec![RackSlot::subtractive_synth(1)];
    // Set gain param to 0.5 for a quieter signal
    if let Some(g) = t.rack[0].params.iter_mut().find(|p| p.id == "gain") {
        g.value = 0.5;
    }
    t.pan = 0.0;
    t.volume = 1.0;
    ...
    p.tracks.push(t);
    let buf = render_to_buffer(&p, 44100, 1.0);
    // L and R should be very close (mono source, center pan)
    ...
    assert!(
        max_diff < 0.01,
        "Center pan mono source: L and R should match, max_diff={:.6}",
        max_diff,
    );
}
